...
class HydraulicFlywheelSystem:
    ...
    def reset(self):
        ...
        self.flow_rate_m3s = 0.0        # **AÑADIDO: Flujo volumétrico actual**
    ...
    def update_state(self, dt):
        ...
        if not self.running:
            self.flow_rate_m3s = 0.0  # **CORREGIDO: Asegurar que el flujo sea 0 cuando está pausado**
            return

        # **CORREGIDO: Motor RPM independiente, no basado en HFW**
        # Simulamos un motor que opera a RPM constante cuando está activo
        if self.mode == "charging":
            self.engine_rpm = ENGINE_RPM_MAX * 0.8  # 80% del máximo durante carga
        else:
            self.engine_rpm = 0.0  # Motor apagado durante descarga
        ...
        if self.valve_state == "pump_to_hfw": # Carga
            ...
            if self.hfw_omega_rad_s > 0:
                ...
                self.flow_rate_m3s = 0 # No hay flujo si está vacío
...
# **CORREGIDO: Variables globales para el sistema**
system_state = None
graphs = None
...
def crear_controles_ui():
    ...
    mode_menu.selected = "Carga (Acumular Energía)"
...
def actualizar_visuales(system_state, models, graphs, ui_elements):
    ...
    models['valve_to_accumulator_line'].color = active_color
    
    # Color de la válvula
    if system_state.valve_state == "pump_to_hfw":
        models['valve_body'].color = color.green
    elif system_state.valve_state == "hfw_to_motor":
        models['valve_body'].color = color.red
    else:
        models['valve_body'].color = color.orange
    ...
    # 4. Gráficas
    graphs['energy_curve'].plot(system_state.time_s, system_state.stored_energy_j / 1000)
...
def main():
    ...
    ui_elements = crear_controles_ui()
