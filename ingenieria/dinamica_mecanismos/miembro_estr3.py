...
# Versión 4: FINAL - Con corrección en el cálculo de M_bb
# ============================================================================
...
# La ecuación de momento correcta suma el momento de reacción M_A y los momentos de las fuerzas aplicadas entre el apoyo y el corte.
eq_aa_M = sp.Eq(M_A_val + (R_Ay_val * x_aa) + F_C * (y_D - y_C) + M_aa, 0)

print("Ecuaciones de equilibrio para el DCL de la seccion aa (parte izquierda) resueltas.")
...
print("Ecuaciones de equilibrio para el DCL de la seccion bb (parte inferior) resueltas.")
...
# ... (El código de los gráficos no cambia) ...
print("="*80)
...
draw_arrow(ax, x_aa, y_D, 200, 0, f'$N_{{aa}}={N_aa_val/1000:.2f}$ kN', 'green')
draw_arrow(ax, x_aa, y_D, 0, -200, f'$V_{{aa}}={V_aa_val/1000:.2f}$ kN', 'green')
...
draw_moment(ax, x_A, y_A, M_A_val, f'$M_A$', 'blue')
draw_arrow(ax, x_C, y_C, 200, 0, f'$F_C$', 'red')
draw_arrow(ax, 0, y_bb, 0, 200, f'$N_{{bb}}={N_bb_val/1000:.2f}$ kN', 'purple')
draw_arrow(ax, 0, y_bb, 200, 0, f'$V_{{bb}}={-V_bb_val/1000:.2f}$ kN', 'purple')
...
ax.axis('off')
texto_resultados = f"""
**RESUMEN DE RESULTADOS CORREGIDOS**
